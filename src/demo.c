    ...
    int frame_skip, char *prefix, char *out_filename, int mjpeg_port, int json_port, int dont_show, int ext_output)
{
    ...
    if (out_filename && !flag_exit)
    {
        ...
        //const char* out_filename = "test_dnn_out.avi";
    ...
    while(1){
         ...
         if (X == 102)
	 {
	   if (full_screen)
	   {  
		cvSetWindowProperty("Demo", CV_WND_PROP_FULLSCREEN, CV_WINDOW_NORMAL);
		full_screen = 0;
	   }
	   else
	   {
...
//		    cvPutText(show_img, "Test", cvPoint(11,40), CV_FONT_HERSHEY_PLAIN, CV_RGB(255, 0,0 ));
                    int c = cvWaitKey(1);
                    if (c == 10) {
